...
python3 -m github-checks.cli init
python3 -m github-checks.cli start-check-run --check-name ruff-checks

# run ruff on ourselves and pass its error messages as annotations to GitHub
ruff check. --output-format=github >> ruff_output.json
python3 -m github-checks.cli add-check-annotations ruff_output.json --log-format ruff-json

# finish the check run and clean up the venv
python3 -m github-checks.cli finish-check-run
deactivate
rm -rf .temp_venv
